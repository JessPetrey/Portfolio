            ...
            <div className="hero-content min-h-screen" ref={props.sectionRef}>
                <div className='hero-content flex-col md:flex-row gap-20 prose'>
                    <div>
                        ...
                        <div className='flex flex-row flex-wrap max-w-md'>
                            {[
                                { icon: 'fa-java', label: 'Java' },
                                { icon: 'fa-js', label: 'JavaScript' },
                                { icon: 'fa-node', label: 'Node.js' },
                                { icon: 'fa-python', label: 'Python' },
                                { icon: 'fa-html5', label: 'HTML' },
                                { icon: 'fa-css3', label: 'CSS' },
                                { icon: 'fa-react', label: 'React' },
                            ].map((item) => (
                                <div className='text-center px-5' key={item.label}>
                                    <i className={`text-6xl fa-brands ${item.icon}`} alt={`Logo for ${item.label}`}></i>
                                    <p>{item.label}</p>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className=' max-w-xs'>
                        <h2 className='text-xl'>Currently working on: </h2>
                        <p>Improving my skills in, and understanding of JavaScript and React.</p>
                        <a href='https://github.com/JessPetrey' rel='noreferrer' target='_blank' className='btn btn-primary'>Check out my GitHub</a>
                    </div>
                </div>
            </div>
        </div>
